...
A vector field $X$ on a Lie group is left-invariant if $l_{g*}X = X$, where $l_g$ is the map given by
\begin{align}
...
$$D(l_g)_{g'}\, (X_g') = X_{g g'}, \, \forall\, g, g' \in G$$
where we wrote explicitly the element $g'$ about which we compute the derivation and the vector field. In the right-hand side the product $gg'$ must be interpreted using the operation of the group $G$. Then, by left-invariance, we have that:
