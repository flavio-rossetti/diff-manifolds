...
where we used: (1) the definition of flow for the flow $\varphi^{-1}$ of the vector field $u$ and the fact that $\psi_s$ is the flow of $v$. And we applied the definitions to the point $\varphi(p)$ instead of $p$ as usual. (2) $(F_*X)_q = dF_{F^{-1}(q)}(X_{F^{-1}(q)})$, see also page 183 \citetalias{Lee}, (3) $\varphi_*^{-1} = \varphi^*$ by definition.
...
and by remark \ref{effectivepullback} we know that the pullback of a $k-$form ($k > 1$) through a constant map is 0, so we also have:
...
\begin{remarkbox}\begin{remark}[Cohomologous forms]
  ...
  Obviously, if $\omega$ and $\omega'$ are exact, we have $\omega = dF$ and $\omega'=dG$ for some ($k-1$)-forms $F$ and $G$. So, the two forms are cohomologous because the $\alpha$ that we need in the definition \eqref{cohomol} is $\alpha = F - G$. All the exact forms are identified with the 0 element of $H^k(U, \rfield)$, because $d_k\omega \sim 0$ (in particular, $d_k\omega = 0$) if $\omega$ is an exact ($k-1$)-form.
...
$$B \equiv \left( \left (\defonde{\phi^i}{x^j} \right)_{x=q} \right)_{i, j = 1, \ldots ,n}, \qquad q = \phi^{-1}(p), \, p \in M \cap V \, .$$
