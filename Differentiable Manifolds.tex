...
\hypersetup{
    colorlinks=true,
    pdftitle={Differentiable Manifolds - Notes},
...
\tcbset{breakable, enhanced, colback=gray!8!white,colframe=gray!15!white}

\newtcolorbox{remarkbox}{breakable, enhanced, colback=white,colframe=gray}
...
\newpage
 These (unofficial) notes are based on the content of the course \textit{Differentiable Manifolds} held at LMU during the Winter Semester 2019/2020 and taught by Prof. \href{https://homepages.physik.uni-muenchen.de/~Ivo.Sachs/}{Ivo Sachs} and Prof. \href{https://www.mathematik.uni-muenchen.de/~tvogel/}{Thomas Vogel}.  Some additional content from books and from my own studies was also added. These notes are neither complete nor accurate. They might contain typos and mistakes. \\[13pt]

 {The first version of these notes was compiled on March 3, 2020. This version of the notes was compiled on \today. For the last version, check \url{https://github.com/fla-io/diff-manifolds}.}
...
\end{equation*}
such that $h(a + b) = h(a) \circ h(b), \forall\, a, b, \in G$, where $(G, +)$ and $(H, \circ)$ are two groups. The definition for rings and fields is analogous. In category theory, (group) homomorphisms are often called (group) morphisms since the morphisms in the category \textbf{Grp} of all groups are set to be group homomorphisms.
\end{definition}
...
\begin{definition}[Equivalence relation]
  ...
  \begin{itemize}
    ...
    \item If $a \sim b$ for $a, b \in E$ $\Rightarrow b \sim a$, \hspace*{\fill}\textit{(symmetric property)}
    \item If $a \sim b, b \sim c $ for $a, b, c \in E$ $ \Rightarrow a \sim c$. \hspace*{\fill}\textit{(transitive property)}
  \end{itemize}
\end{definition}
...
\begin{enumerate}
	...
	\item $\mathcal{O}$ is closed under finite intersections: $U_1, \ldots, U_k \in \mathcal{O}$ for $k \in \mathbb{N}$, then $U_1 \cap \ldots \cap U_k \in \mathcal{O}$.
\end{enumerate}
...
\item "$M \setminus \{x\} \supseteq \cup_{y \in M \setminus \{x\}} U_y$": if $z \in \cup_{y \in M \setminus \{x\}} U_y$, then $z \not = x$, otherwise there would be an intersection between an open set $U_{\bar x}$ for a fixed $\bar x$ and every open set containing $x$. This is impossible because by Hausdorff assumption there exists at least one open set in this family that does not intersect $U_{\bar x}$.
\end{itemize}
...
Let $(M_i, \mathcal{O}_i), i \in I$ be topological spaces. Then the product topology on $\prod_{i \in I} M_i$ is the smallest topology such that the projections
$$
...
Why? Because\footnote{the proof is easier to follow if you think of the space $M$ as $\rfield$.}: given $a, b$ in $M$, with $ a \not = b$, if $U_a$ and $U_b$ are open sets that contain $a$ and $b$ respectively, the complements of $U_a$ and $U_b$ must be finite (so, $U_a$ and $U_b$ must be infinite, because the bigger space $M$ is infinite).
...
\begin{itemize}
\item It cannot be written as union of two non-empty, disjoint closed sets, or
\item $X = A \cup B$ with $A, B$ disjoint and open sets implies that $A= \emptyset$ or $B = \emptyset$, or
...
$$\restrict{v(fg)}{a} = f(a) \restrict{v(g)}{a} + g(a) \restrict{v(f)}{a} \, , \quad \forall\, f, g \in C^{\infty}(\rfield^n).$$
\end{definition}
...
\begin{tcolorbox}\begin{example}
  ...
  \begin{enumerate}
    ...
    \begin{align} \label{dualisomorph}
      ...
      x &\mapsto g(x, \cdot) \nonumber
    \end{align}
    where $g$ is the Euclidean metric in $\rfield^3$ (in the sense of Riemannian geometry), which in $\rfield^n$ coincides with the standard dot product: $g(x, y) = \sum_i x^i y^i$.
...
\end{matrix}
  \right)$$
  and a dual basis is given by $e^{*i}(\cdot) = \frac{1}{2} \tr(e_i, \cdot)$ (using the matrix product).
  \item Quantum Mechanics: $$V = \{ f \colon \rfield^3 \rightarrow \rfield \, | \, \|f\|^2 = \int_{\rfield^3} |f|^2 d^3x < \infty \} = L^2(\rfield^3)$$ is a vector space. If we consider the Laplacian operator $\Delta = \frac{\partial^2}{(\partial x^1)^2} + \frac{\partial^2}{(\partial x^2)^2} + \frac{\partial^2}{(\partial x^3)^2}$, then a basis is given by $\{e_n\} = \{\frac{f_n}{||f_n||}\}$, where $f_n$ are eigenfunctions of $\Delta$: $\Delta f_n = \lambda_n fn$ (if we choose appropriate boundary conditions).
  A dual basis is given by $e^{*n}(\cdot) = \int_{\rfield^3} e_n (\cdot)$
  Notice that both $V$ and $V^*$ are infinite-dimensional spaces.
  In quantum mechanics the following notations are also used:
...
Every vector in $\rfield^n$ about a point $p \in \rfield^n$ (i.e. such that its origin is the point $p$) "can be seen" as a derivation (cf. def. \ref{derivation}), i.e. as a directional derivative of a function evaluated at the point $p$. For the sake of simplicity, let us now consider $p=0$ (but the following results are true $\forall \, p \in \rfield^n$).
By "can be seen" we mean that there is an isomorphism $\psi$ associating such vectors to such linear forms. Let's construct this isomorphism in the following steps:
...
\begin{remarkbox}\begin{remark} [Fields vs. Derivations] \label{fieldsvsder}
  ...
  However, definition \ref{setsfields} is a bit different. Why?
  The fact is, we can consider a vector of $\rfield^n$ as a directional derivative, cf. remark \ref{vectorasder} again (we are not considering any fixed point here, but the results do not change). Now, a derivation, as defined in def. \ref{derivation}, is a map $v \colon C^{\infty}(\rfield^n) \rightarrow \rfield$, i.e. we can say that a derivation is a very smooth element of $\mathbb{F}(\rfield^n) = \{ \text{functions } f \colon \rfield^n \rightarrow \rfield\}$. Hence, there exists a correspondence between $\mathfrak{X}(\rfield^n)$ and $X(\rfield^n)$, modulo the required regularity.
  ...
  \end{equation}
  where we used  the summation convention, and the fact that every basis vector $e_i$ can be seen as $\restrict{\partial_{x_i}}{p}$. We defined it in the right way because, as expected, we found that $v_p(f)$ is the directional derivative of $f$ in the direction of $v$, evaluated at $p$. So, in brief:
  \begin{itemize}
...
Given a vector space $V$ on a field $\mathbb{K}$, with $\dim(V) = n$, and with $k \le n$, an \textit{exterior form of degree $k$} (or $k$-linear form, or $k$-form) is a multi-$\mathbb{K}$-linear map
\begin{equation*}
...
\end{equation}
for all permutations $\pi$ of $k$ elements, i.e. $\pi \in S_k$, where $\sgn(\pi)$ is the sign of the permutation.
...
\begin{itemize}
\item Linear independence: We have to show that
$$0 = \sum\limits_{i_1 < \ldots < i_k} a_{i_1 \cdots i_k} e^{*i_1} \wedge \ldots \wedge e^{*i_k}$$
implies $a_{i_1 \cdots i_k} = 0, \forall\, i_1, \ldots, i_k$. Indeed, if we apply $(e_{j_1}, \ldots, e_{j_k})$ on the above equation, we obtain
\begin{align*}
0 &= a_{j_1 \cdots j_k} \underbrace{(e^{*j_1} \wedge \ldots \wedge e^{*j_k}) (e_{j_1}, \ldots, e_{j_k})}_{= 1} + \\
& + \sum\limits_{\substack{j_1 < \ldots < j_k \\
i_1 \not = j_1, i_2 \not = j_2, \ldots}} a_{i_1 \cdots i_k} \underbrace{(e^{*i_1} \wedge \ldots \wedge e^{*j_k}) (e_{j_1}, \ldots, e_{i_k})}_{= 0} = a_{j_1 \cdots j_k} \, .
\end{align*}
The second term is zero because $(e^{*i_1} \wedge \ldots \wedge e^{*i_k}) (e_{j_1}, \ldots, e_{j_k}) = \det \left (e^{*i_l} (e_{j_m}) \right)$. Indeed, the matrix $e^{*i_l} (e_{j_m})$ has null coefficients on the main diagonal (because $j_1 \not = i_1, j_2 \not = i_2, \ldots$) and one of the rows must be 0 because of  the ordering $j_1 < \ldots < j_k$ and $i_1 < \ldots < i_k$ (e.g. consider the case $j_2 = i_1, \ldots$). Thus, the determinant is 0.
Therefore, $a_{j_1 \cdots j_k} = 0$ and the indices $j_1, \ldots, j_k$ were chosen arbitrarily.
...
\begin{tcolorbox}\begin{example}[Oriented area] \label{orarea}
  ...
  \begin{enumerate}
    \item Let's suppose our space is $V = \rfield^4$ with cartesian coordinates.
...
Here are some interesting properties of $k$-forms:
\begin{proposition}
...
$ $
	\begin{enumerate}
	\item Writing the forms in terms of the basis, we only need to prove that property (1) holds for the elements of the dual basis. Moreover, by multilinearity, we just need to prove it for the elements of the dual basis applied to a basis $\{e_i\}$ of $V$. Thus, the proof for (1) is over because this property is true for the determinant: when we compute it, we can start from any row or column.
...
\begin{remarkbox}\begin{remark}
  To show the equivalence of the two definitions of differential $k$-forms we just need to show that:
  \begin{equation}
    \omega(p)(v_1, \ldots, v_k) = \omega(v_1, \ldots, v_k)(p) \, .
  \end{equation}
  See also exercise 1, problem sheet 3.
\end{remark}\end{remarkbox}

We want to generalize the concept of a differential of a function.
...
\begin{remarkbox}\begin{remark}[Differential expression in coordinates] \label{diffcoordremark}
 ...
 \end{equation}
   ...
   \end{equation}
   On the other hand, if we apply the above to $f = x^j$ (coordinate function, as defined in \eqref{coofunc}), we get:
   \begin{equation}
...
\begin{tcolorbox}\begin{example}[Computation of $d\omega$]
  ...
  \begin{enumerate}
    ...
    where we used that "$d (dx^1 \wedge dx^2) = 0$" because there is no non-zero 3-form on a 2-dimensional space. (otherwise, we can use that $d^2=0$, but we still have to prove it!)
    \item In $\rfield^n, n \ge 2$, let's consider:
    $$\omega = x^2 dx^1 \, .$$
    We have:
    $$ d \omega = dx^2 \wedge dx^1$$
    where we used the definition \ref{extder}. Moreover, if $u,v$ are vector fields, then by definition of exterior product we have:
...
\begin{remarkbox}\begin{remark}
  The exterior derivative increases the degree of a $k$-form by 1 (the $k$-form becomes a ($k+1$)-form). Can we get backwards, which is, can we decrease the degree of a $k$-form? The answer is yes, and is given by the interior derivative, which we now introduce.
\end{remark}\end{remarkbox}
...
\begin{definition}[Interior derivative] \label{intder}
  ...
  \end{align}
  such that
  $$(i_z \omega) (v_1, \ldots, v_{k-1}) = \omega(z, v_1, \ldots, v_{k-1}), \quad \forall \, v_1, \ldots, v_{k-1} \in \mathfrak{X}(\rfield^n) \, .$$
...
\begin{enumerate}
\item $\sum\limits_{\substack{i_1 < \ldots < i_k \\ j_1 < \ldots < j_p }} a_{i_1 \cdots i_k} b_{j_1 \cdots j_p} i_z \left( dx^{i_1} \wedge \ldots \wedge dx^{i_k} \right ) \wedge dx^{j_1} \wedge \ldots \wedge dx^{j_p}$, because the only non-zero term comes from $i_z(e^{*1}) = e^{*1}(z) = z_1 e^{*1}(e_1) = z_1$ (locally), and we know that the non-zero term must be hidden in the first $k$ wedge products, or
...
\begin{remarkbox}\begin{remark}
  In the above proposition there is a little abuse of notation when we claimed $i_z(i_z \omega) = 0$, see also remark \ref{abusenot1}.
\end{remark}\end{remarkbox}
...
\begin{definition}[Pullback]
  ...
  such that
  \begin{equation*}
    (f^* \omega)(p) (u_1, \ldots, u_k) = \omega (f(p)) (df_p(u_1), \ldots, df_p(u_k)), \quad \forall\, \, p \in \rfield^n, \forall \, u_1, \ldots, u_k \in \mathfrak{X}(U) \, .
  \end{equation*}
\end{definition}

Now, we want to generalize the concept of differential of a function.
...
By definition of differential: $df_p(v) = v_p(f)$, where $v_p$ %p's added (and below)
is a vector tangent to $\rfield^n$ at $p$. Since vectors are like directional derivatives, $v_p(f)$ is the directional derivative of $f$ with respect to $v_p$ (not evaluated at any point, for now!). In particular, if we apply the definition to a function $h \in C^{\infty}(\rfield^n, \rfield^m)$, we have (cf. also p. 55 \citetalias{Lee}): 
$$(f_*)_p (v)(h) \overset{(1)}{=} v(h \circ f) = v(h(f)) \overset{(2)}{=} v (f^* h) $$
where we used: (1) $(f_*)_p (v)(h)  = v_p(h \circ f)$, it can be proved that it is the right expression in a similar way to what we did to prove that $(f_*)_p (v)= v_p(f)$ is the right expression (cf. remark \ref{diffcoordremark}), (2) the expression of the pullback for a differentiable function (i.e. a 0-form).
\end{remark}\end{remarkbox}
...
\begin{enumerate}
\item \begin{align*}
f^*(\omega + \varphi) &= (\omega + \varphi) (f_*(u_1), \ldots, f_*(u_k)) =\\
& = \omega (f_*(u_1), \ldots, f_*(u_k)) + \varphi (f_*(u_1), \ldots, f_*(u_k)) = \\
&=  f^*(\omega) + f^*(\varphi) \, .
\end{align*}
...
\begin{definition}[Tangent space]
  Let $U \subset \rfield^n, U$ open set. Given $p \in U$,
  the set of all derivations of $C^\infty(U)$ at $p$ 
  (cf. def \ref{derivation}) is called tangent space to $U$ at $p$ and is denoted by $T_p U$. An element of $T_p U$ is called a tangent vector at $p$, and it is often denoted by $(p,v)$, $v_p$, or just $v$.
\end{definition}

There are alternative equivalent definitions of the tangent space. Some of them will be analyzed in the next sections.
...
\begin{remarkbox}\begin{remark} \label{disjointrem}
  In the previous definition, the "$\sqcup$" symbol denotes a disjoint union. 
  The disjoint union of two elements $x,y$ can be written as $\{(1, x), (2, y)\}$, where we introduced an artificial index in order to distinguish $(1,x)$ from $(2,y)$ even if $x=y$.
\end{remark}\end{remarkbox}
...
\begin{remarkbox}\begin{remark} [Space of sections]
  The set of all vector fields $\mathfrak{X}(U) \equiv \{v \in C^\infty(U, TU) \, | \, \pr(v_p) = p \}$ is also called the space of sections in $TU$. Thus, a section of $TU$ is a vector field.
\end{remark}\end{remarkbox}
...
\begin{definition}[Abstract Lie algebra]
  ...
  \begin{itemize}
    ...
    $$[[u, v], z] + [[z, u], v] + [[v, z], u] = 0 \, \forall \, u,v,w \in V \, .$$
  \end{itemize}
\end{definition}
...
\begin{tcolorbox}
\begin{example}
	$\rfield^3$ equipped with the usual vector product and the vector space of matrices with the matrix commutator $[A,B] = AB-BA$ are typical Lie algebras.
\end{example}
\end{tcolorbox}

\begin{remarkbox}
	\begin{remark}[Lie bracket for vector fields]
		The vector space $V=\mathfrak{X}(\rfield^n)$ can be equipped with a canonical Lie bracket. For $u,v \in \mathfrak{X}(\rfield^n)$ we set for $f \in C^\infty(\rfield^n)$
		\begin{align*}
			[u,v](f) = uv(f) - vu(f).
		\end{align*}
		In fact, $[u,v]$ is a vector field as it satisfies the Leibniz rule:
		\begin{align*}
			[u,v](fg) &= uv(fg) - vu(fg) \\
			&= u(gv(f) + fv(g)) - v(gu(f) + fu(g)) \\
			&= g uv(f) + f uv(g) - gvu(f) - fvu(f) \\
			&= g [u,v](f) + f[u,v](g)
		\end{align*}
		It is easily verified that $(\mathfrak{X}(\rfield^n), [\cdot,\cdot])$ is indeed a Lie algebra.
	\end{remark}
\end{remarkbox}

\begin{definition}[Integral curve]
  An integral curve for a vector field $v$ is a smooth curve $\phi \colon (a, b) \rightarrow \rfield^n$ satisfying $\dot\phi(t) = v_{\phi(t)}$ ($v_{\phi(t)}$ is the vector tangent at $\phi(t)$ for $t$ fixed, remember the previous notation!).
\end{definition}

\begin{lemma}[Translation lemma]
	If $\phi \colon (a, b) \rightarrow \rfield^n$ is an integral curve of the vector field $v$, then for all $c\in \rfield^n$ the curve $\tilde{\phi} \colon (a+c, b+c) \rightarrow \rfield^n$ defined by $\tilde{\phi}(t) = \phi(t+c)$ is also an integral curve of $v$.
\end{lemma}

\begin{proof}
	We simply check the defining property of an integral curve
	\begin{align*}
		\dot{\tilde{\phi}}(t) = \dot{\phi}(t+c) = v_{\phi(t+c)} = v_{\tilde{\phi}(t)}
	\end{align*}
	where in the second step we used that $\phi$ is an integral curve for $v$.
\end{proof}
...
\begin{definition}[Flow]
  ...
  i.e. for all $p \in \mathbb{R}^n$ the flow $\theta$ satisfies 
  \begin{align}
    \dot\theta_t(p) &= v_{\theta_t(p)} \\
    \theta_0(p) &= p.
  \end{align}
 \end{definition}
...
\begin{remarkbox}
	\begin{remark}
		The translation lemma above implies that $t \mapsto \theta_{t+s}(p)$ is an integral curve starting at $q = \theta_s(p)$. By uniqueness of the integral curve we obtain $\theta_t(q) = \theta_{t+s}(p)$, i.e. $\theta_t \circ \theta_s = \theta_{t+s}$. This property together with $\theta_0(p) = p$ for all $p$ can be used as a definition of the flow. 
	\end{remark}
\end{remarkbox}

\begin{remarkbox}
	\begin{remark}
		Each flow defines locally a vector field $v$ given pointwise by
		\begin{align*}
			v_p = \restrict{\frac{d}{dt}}{t=0} \theta_t(p).
		\end{align*}
		The vector field $v$ is called the infinitesimal generator of $\theta$. For fixed $p$ each curve $\theta_t(p)$ is an integral curve for $v$.
	\end{remark}
\end{remarkbox}

\begin{definition}[Lie derivative of a $k$-form]
  Let $z \in \mathfrak{X}(\rfield^n)$ be a differentiable vector field, $\theta_t$ its flow and $\omega \in \Omega^k(\rfield^n)$. Then the Lie derivative of $\omega$ is defined as
  \begin{equation}
    L_z \omega = \restrict{\frac{d}{dt}(\theta_t^* \omega)}{t=0} \, .
  \end{equation}
\end{definition}

\begin{remarkbox}\begin{remark}
  We denoted the flow by the symbol $\theta_t$ and not $\theta$. What we are doing here is not caring about $p$: $\theta^*_t \omega (\cdot) = \omega(\theta_t(\cdot))$.
...
\begin{definition}[Pullback of a vector field through a diffeomorphism] \label{pulldiffeom}
  ...
  In particular, given a flow $\phi$ and $t$ fixed, $\phi(t, \cdot) = \phi_t(\cdot)$ is a diffeomorphism on $\rfield^n$ with $\phi^{-1} (t, \cdot) = \phi(-t, \cdot) = \phi_{-t}(\cdot)$.
\end{definition}
...
\begin{lemma} \label{lemmadiff1}
  Let $u, v$ be smooth vector fields on $\rfield^n$ and $\varphi \in \Diff(\rfield^n)$. Let $\phi$ be the flow of $u$ and let $\psi$ be the flow of $v$. Then
  \begin{itemize}
...
\begin{remarkbox}
	\begin{remark}
		The above lemmata show that two vector fields $u,v$ commute, i.e. $[u,v] = 0$, if and only if their corresponding flows $\phi, \psi$ commute, i.e. $\phi_t \circ \psi_s = \psi_s \circ \phi_t$ for all $s,t \in \rfield$.
	\end{remark}
\end{remarkbox}
...
\begin{remarkbox}
	\begin{remark}
		From lemma \ref{commlemma} and the correspondence $L_u v = [u,v]$ one can easily obtain non-trivial properties for the Lie derivative, e.g. (notation as above)
		\begin{enumerate}
			\item $L_u v = -L_v u$,
			\item $L_u(fv) = fL_u v + u(f)v$,
			\item $\varphi^* L_u v = L_{\varphi^* u} (\varphi^*v)$.
		\end{enumerate}
	\end{remark}
\end{remarkbox}
...
\begin{itemize}
  ...
  $$\int_U d\omega = \int_U (\partial_{x^1} a_2) dx^1 \wedge dx^2 + (\partial_{x^2} a_1) dx^2 \wedge dx^1 = \int_{\partial U} \omega$$
  where we used the fundamental theorem of calculus.
  \item More generally, if $U$ is a compact subset of $\rfield^2$ with piecewise regular boundary $\partial U$ (piecewise homeomorphic to intervals in $\rfield$) then, by decomposing $S$ in terms of little squares and interpreting $\int_U d\omega$ as a Riemann sum over the square, we obtain the result
  ...
  \item More generally, the parametrization of $\partial M$ may be different from the one induced by $M$. Then we have:
...
\begin{proposition} \label{propex1forms}
  The following are equivalent (\textbf{note that we are considering 1-forms!}):
  \begin{enumerate}
...
\end{proposition}
\begin{proof}[Sketch of the proof]
First, $(1) \Rightarrow (2)$ because of Stokes' theorem. Moreover, $(2) \Rightarrow (3)$ using Stokes' theorem again. Now, $(3) \Rightarrow (2):$ the curve made of the union of $\gamma_1$ and $\gamma_2$ is a closed curve (see Fig. \ref{Fig:closedcurve}). Then, if $\gamma$ is the name of such a closed curve:
$$0 = \int_{\gamma} \omega = \int_{\gamma_1} \omega - \int_{\gamma_2} \omega$$
where the minus sign comes from the orientation of the curves.
Then: $\int_{\gamma_1} \omega = \int_{\gamma_2} \omega$ where $\gamma_1$ and $\gamma_2$ are any two curves with the same endpoints. It means that we can consider the integral of $\omega$ on any curve and the result  doesn't change \textbf{if} the new curve has the same endpoints of the initial curve.
\begin{figure}[H] 
     \centering
     \includegraphics[width=0.3\textwidth]{Images/closedcurve.pdf}
     \caption{Union of $\gamma_1$ and $\gamma_2$} \label{Fig:closedcurve}
\end{figure}
...
\begin{remarkbox}\begin{remark}[A closed form is not always exact]
  If $\omega$ is exact, then it is closed (because $d^2=0$). But not every closed form in $\Omega^1(U), U$ open subset of $\rfield^n$ is exact. Cf. $\omega = - \frac{y}{x^2 + y^2} dx + \frac{x}{x^2+y^2}dy$ in $\rfield^2$ minus the origin. If $\gamma$ is a closed curve around the origin of $\rfield^2$, we have:
...
\begin{theorem}[Poincaré theorem for 1-forms on $\rfield^n$]
  Let $\omega \in \Omega^1(U), U \subset \rfield^n, U$ open. Then $d\omega = 0$ if and only if for each $p \in U$ there is a neighbourhood $V \subset U$ of $p$ and a differentiable function $f \colon V \rightarrow \rfield$ such that $\omega = df$, i.e. $\omega$ is closed if and only if $\omega$ is locally exact.
\end{theorem}
...
\begin{remarkbox}\begin{remark}
  ...
  Using the Poincaré theorem for 1-forms, we can extend the definition of the integral of a closed 1-form along a \textbf{continuous} path (until now, we have always assumed the our paths were piecewise differentiable). In fact, assume that $ \omega \in \Omega^1(U), d \omega = 0$, and $\gamma$ such that:
  ...
  $$\int_{\gamma} \omega = \sum_i \left[ f_i(t_{i+1}) - f_i(t_i) \right ] \, .$$
  The integral of $\gamma$ is well defined because the definition is independent from the choice of our partition: if $P$ is a partition and $P'$ is a refinement of $P$ (i.e. it is the same partition plus an extra point $t' \in (t_i, t_{i+1})$ for some $i$), then:
  $$[f_i(t_{i+1}) - \cancel{f_i(t')}] + [\cancel{f_i(t')} - f_i(t_i)] = [f_i(t_{i+1}) - f_i(t_i)] \, .$$
  Then the integral does not change if we consider a refinement. If we consider a general partition $P'$, we can add every point of the partition $P$ to $P'$, so that we get a refinement of $P'$ that we will call $P''$. The integral on the partition $P'$ has the same value of the integral on the partition $P''$ by the above argument. Now, we can add every point of the partition $P'$ to $P$, so to get the partition $P''$ again, but now we can see $P''$ as a refinement of $P$. Then the integral on $P$ and on $P''$ are the same. Then, the integrals on $P$ and $P'$ are also the same.
\end{remark}\end{remarkbox}
...
\begin{proof}
  Let's choose coordinates $\{x^1, \ldots, x^n, t\}$ for $U \times \rfield$. Then we write $\bar \omega$ in the basis:
  ...
  (We can always do this, the coefficients could also be trivial). We set $\omega_1$ to be the first and $dt \wedge \eta$ to be the second summand. Obviously, we have $i_{\partial_t} \omega_1 = 0$ and $i_{\partial_t} \eta = 0$ by the definition of the dual basis: $dx^{j} (\partial_t) = 0, \forall \, j$, $dt(\partial_t) = 1$. This decomposition is unique since it is independent of the coordinates chosen on $U$.
\end{proof}
...
Let's define the map
  \begin{align}
    ...
    \eta &\mapsto I\eta \nonumber
  \end{align}
  such that $$(I\eta)_p(z_1, \ldots, z_{k-1}) = \int_0^1 \eta(p, t)(\partial_t, i_{t}^*z_1, \ldots, i_{t}^*z_{k-1})dt$$
...
%Indeed, since $H \circ i_1 = Id$ and $H \circ i_0 = p_0$, $\forall \, p \in U$ we have
  ...
  Let's consider $p \in U$ with coordinates $x^1, \ldots, x^n$. It suffices to check the claim for the summands in the decomposition $\bar \omega = \omega_1 + dt \wedge \eta$ due to linearity.
  \begin{description}
  \item[case a)] Assume:
  ...
  Moreover, $I\bar \omega = 0$ because $\bar \omega$ does not contain any $dt$ in this case.
  \item[case b)] Assume:
...
Then, we can extend the Poincaré lemma to $k$-forms:
\begin{theorem} [Poincaré Lemma] \label{poinclemma}
...
\begin{theorem}
  Let $U$ be an open, contractible set. Let $\omega \in \Omega^1(U)$, with $d \omega = 0$ (closed), and $\gamma_1, \gamma_2$ be two homotopic curves in $U$ with the same endpoints. then:
  \begin{equation}
...
\end{figure}
$I$ is contractible, then by Poincaré lemma \ref{poinclemma}, $\omega$ is exact in $I$. Using Stokes's theorem and denoting the curve $\gamma_1$ with reversed orientation by $\hat \gamma_1$:
...
\begin{proposition}
  Let $U$ be an open, contractible set. If $\omega$ is a closed 1-form on $U$, $\gamma_1$ and $\gamma_2$ two closed curves, freely homotopic in $U$, then:
  \begin{equation}
    \int_{\gamma_1} \omega = \int_{\gamma_2} \omega \, .
  \end{equation}
...
We can decompose the space into small squares and use the Stokes' theorem for each of them.
\end{proof}
...
The property $d_{k+1} \circ d_k=0$ implies $\im(d_k) \subset \ker(d_{k+1})$.
\end{definition}

\begin{definition}[Exact sequence]
Let $A^*$ be a complex. It is called an exact sequence if $\im(d_k) = \ker(d_{k+1})$ for each $k$.
\end{definition}
...
It is well defined since $\im(d_k) \subset \ker(d_{k+1})$ by definition of cochain complex. If we are dealing with a chain complex, it is called homology group instead of cohomology group. 
The $k$-th cohomology group is a quantitative measure for the failure of exactness at $A^k$.
\end{definition}

\begin{definition}[de Rham complex]
  The de Rham complex is the sequence
  $$0 \overset{d}{\longrightarrow} \Omega^0 \overset{d}{\longrightarrow} \cdots \overset{d}{\longrightarrow}   \Omega^{k-1} \overset{d}{\longrightarrow} \Omega^{k} \overset{d}{\longrightarrow} \Omega^{k+1} \overset{d}{\longrightarrow} \cdots \overset{d}{\longrightarrow} \Omega^{n} \overset{d}{\longrightarrow} 0 $$
  where $d$ is the exterior derivative now. It is well defined since we know that $d^2 = d \circ d = 0$ from the properties of the exterior derivative. Thus, $\im(d_k) \subset \ker(d_{k+1})$. Notice that we wrote $\Omega^k$ instead of $\Omega^k(U)$ for the sake of simplicity, and that the sequence stops at $\Omega^n$ because we are assuming $U \subset \rfield^n$.
\end{definition}
...
\begin{remarkbox}\begin{remark}[Cohomologous forms]
  The set $\ker(d_{k+1})$ contains all the $k$ forms that are closed. The set $\im(d_k)$ contains the exact forms. As usual when we deal with vector spaces, the equivalence relation of the quotient module we considered is:
...
\begin{tcolorbox}\begin{example}
  Let's consider the following examples:
  \begin{itemize}
    \item If $U$ is contractible, then $H^k(U, \rfield) = \{0\}$ by the Poincaré lemma,
    ...
    \item Two-dimensional torus $T^2$: $H^0(T^2) = \rfield$,  $H^1(T^2) = \rfield \oplus \rfield$, $H^2(T^2) = \rfield$.
  \end{itemize}
\end{example}\end{tcolorbox}
...
\begin{remarkbox}\begin{remark} [Exact sequence] \label{exsequence}
  If $M \subset \rfield^n$ and $H^k(M, \rfield) = 0$ for $k=1, \ldots, n$, then
    ...
    is such that $\ker(d_{k+1}) = \im(d_k)$ for every $k=1, \ldots, n$. Notice that for $k=0$, $\Omega^0 = \{\text{functions}\}$, we have $H^0 \ne 0$. 
\end{remark} \end{remarkbox}

\begin{definition}[de Rham cohomology ring]
  Let $M \subset \mathbb{R}^n$. The de Rham cohomology ring is defined as
  \[ H^*(M, \rfield) = \bigoplus_{k=0}^n H^k(M, \rfield) \, .\]
\end{definition}

\begin{tcolorbox}\begin{example}
  $H^*(T^2, \rfield) = \rfield \oplus \rfield$, where $T^2$ is the two-dimensional torus.
\end{example}\end{tcolorbox}

\begin{remarkbox}\begin{remark}
  If $\phi \colon U \rightarrow V$, $U, V$ open 
  subsets of $\rfield^n$, $\phi$ diffeomorphism, then $\phi^* \colon \Omega^k(V) \rightarrow \Omega^k(U)$ is the pullback of $\phi$ on $k$-forms.
  By the construction above, we can also define such pullbacks 
  on $H^k$, as $\phi^* \colon H^k(V) \rightarrow H^k(U)$, which maps closed-but-not-exact forms to closed-but-not-exact forms due to the naturality of the pullback. 
  If $\phi$ is a diffeomorphism, then the pushforward $\phi_*$ is an isomorphism and so is $\phi^*$, due to the fact that $\phi^* = \phi_*^{-1}$.
...
\begin{lemma}[Zig-zag lemma]
  Let's consider $\phi^*, \psi^*$ as in the previous definitions. Then there exists a linear map $\delta$, called the connecting homomorphism, such that:
  \begin{align*}
...
\textbf{Premise:} we want to explore more general spaces than $\rfield^n$, and analyze more general topologies as well. For now, we can consider generalizations of curves and surfaces in $\rfield^n$, whereas a more abstract definition will come later. Note that what we studied until now can often be extended to manifolds: under some conditions, manifolds can be "embedded" in $\rfield^n$.
...
\begin{description}
\item[\ref{sub1})] The homeomorphism hypothesis is quite intuitive: we want our space to be locally similar to $\rfield^n$. Indeed, the fact that $\phi$ is an isomorphism makes the vector field given by the differential of $\phi$ well-defined: the map $\phi$ determines a vector field on $M$ through its differential $d\phi=\phi_*$, if $\phi_*$ is a bijective map.
\begin{center}
...
\end{align*}
By the proof of (a) $\Rightarrow$ (b) of theorem \ref{allequiv}, we know that there exists a diffeomorphism $\Phi \colon \rfield^n \supset W \rightarrow V'' \subset \rfield^n$, with $(q, 0) \in W$, such that:
...
where the right-hand side is smooth. We can prove the same result the other way around.
\end{proof}
...
Now, we want to use diffeomorphisms like those in corollary \ref{corsubmanifold} in order to introduce the concept of manifold.
...
\begin{remarkbox}\begin{remark}
  The definition \ref{manifoldprel} above is "\textit{preliminary}" because we have not specified anything about the topology yet (we are still working in the Euclidean topology setting). Moreover, in every equivalence class there exists a unique maximal atlas (i.e. such that, if combined with an other atlas, it can't get any larger). We also notice that if the charts are smooth enough, we can pullback "everything" (e.g. all our vector fields, differential forms, etc.) to $\rfield^n$. However, in that case, everything is defined \textit{locally}. In order to patch them all together, we need some other result (like the partition of unity, see below).
\end{remark}\end{remarkbox}
...
\begin{tcolorbox}\begin{example}[Projective space]
  ...
  We notice that $\mathcal{A}$ is an atlas because $\phi_i$ satisfies the properties of  definition \ref{atlasdef}:
  \begin{itemize}
...
\begin{remarkbox}\begin{remark}[Extension of analysis to manifolds]
  The existence of an atlas allows us to define concepts of vector-space analysis in the setting given by manifolds. For instance, we can define continuous and smooth functions on manifolds using the concepts of continuity and smoothness that we use in $\rfield^n$.
\end{remark}\end{remarkbox}
...
\begin{remarkbox}\begin{remark}[Representation of $v$ in $U_{\alpha}$, local coordinates]
  If $(\phi_{\alpha}, U_{\alpha})$ is a chart, with $U_{\alpha} \subset \rfield^n$, $\phi_{\alpha}$ is called a local coordinate map. Then, the representation of a vector field $v$ in $U_{\alpha}$ is:
...
\begin{definition} [Orientable manifold]
  ...
  $$
    (\phi_{\beta}^{-1} \circ \phi_{\alpha})_* \colon  U_{\alpha} \cap U_{\beta} \rightarrow U_{\alpha} \cap U_{\beta}
  $$
...
\begin{definition}[\textbf{Preliminary} definition of diff. manifold with boundary]
  An $n$-dimensional differentiable manifold with a regular boundary is a set $M$ with an equivalence class of atlases, as usual, but with the difference that $\rfield^n$ in the definition \ref{atlasdef} is replaced by $H^n$ everywhere. The boundary is "regular" if it is described by a regular curve (no intersections, etc...).
\end{definition}
...
\end{theorem}
\begin{proof}[Idea of the proof]
First, we prove the theorem for a rectangle and then, for a generic set, we approximate with rectangles.
\end{proof}
...
$$\varphi_i \circ \varphi_j^{-1} \colon \varphi_j(U_i \cap U_j) \rightarrow \varphi_i (U_i \cap U_j)$$ are smooth (i.e. $C^{\infty}$). 
\end{definition}

Sometimes we consider $\varphi$ going from $\rfield^n$ to $M$ instead of the other way around, without any loss of generality. Notice that we are considering equivalence classes of atlases. We recall that the equivalence relation is the following one: two atlases are equivalent if their union is still a smooth atlas.
...
A derivation $v_p$ on smooth functions $C^{\infty}(M)$ at $p$ is a $\rfield-$linear map $v_p \colon C^{\infty}(M) \rightarrow \rfield$ which satisfies the Leibnitz rule $\restrict{v(fg)}{p} = \restrict{v(f)}{p} g(p) + f(p)\restrict{v(g)}{p}$.
\end{definition}

\begin{definition}[Alg. tangent space]
We define the algebraic tangent space at $p$ as:
\begin{equation*}
\tangentalg{p} M = \{\text{derivations at } p \} \, .
\end{equation*}
\end{definition}
...
\end{equation}
where $v$ is defined such that, for every couple of charts $(V, \psi)$ and $(U, \varphi)$ around $p$:
...
The three previous definitions of tangent space are all equivalent, indeed we can find isomorphisms between such spaces.
\begin{enumerate}
\item
\begin{align}
...
\item The pullback of $f$ is a map $f^* \colon \Omega^k(N) \rightarrow \Omega^k(M)$, for some $k \in \mathbb{N}$, $k \le \dim M$.
\end{itemize}
\end{remark} \end{remarkbox}
...
where $D(l_g)_{g'}$ means that the derivation is computed at $g'$ (we use this notation instead of $D_{g'}(l_g)$) and $g'$ is also the point at which we consider our vector field $X$. At the right-hand side, the product $gg'$ must be interpreted using the operation of the group $G$. Then, by left-invariance, we have that:
...
\end{equation}
What if we want to compute the commutator of two left-invariant vector fields? We have that $[X, Y] = \text{ad}(X)(Y)$, where "$\text{ad}$" is the map defined in \eqref{grouphom2}. This is true in general, not only for $G \subset GL(n, \rfield)$. In fact, let's recall the maps defined in remark \ref{autremark}:
\begin{enumerate}
\item \begin{align}
c_g \colon G &\rightarrow G \\
h &\mapsto ghg^{-1} \nonumber
...
It is an open neighbourhood, since it is an intersection of open sets, with
...
\end{equation}
And we have already considered the Poincaré lemma \ref{poinclemma}:
...
Now, we want to discuss about $k$-forms with compact support. We define the support of $\omega$ as $\supp(\omega) \equiv \overline{ \{ p \in M \, | \, \omega(p) \ne 0\} }$. So, a $k$-form $\omega$ has compact support if $\supp (\omega)$ is a compact set. First, we notice that the exterior derivative $d$ maps forms with compact support to forms with compact support. Then, it is natural to define the following.
...
Now, we want to prove a compactly supported version of the Poincaré lemma. In order to do that, we need some general result on manifolds.
...
\end{equation}
which actually maps compactly supported forms to compactly supported forms. So, if now $\omega \in \Omega^k_c(M \times \rfield)$, then $\pi_* \omega$ has compact support as a $(k-1)$-form in $M$. Such a map is called \textit{integration along the fiber}. We used the notation of the pushforward because it is indeed another way to write the pushforward of a $k$-form.
We also notice that, because of a generalized version of lemma \ref{lemmaform1}, every $k$-form on $M \times \rfield$ can be written as a sum (with more than two summands in general) of two types of $k$-forms:
\begin{description}
...
\item(Type A):
\begin{align*}
\pi_* \circ d \left ( f(x, t) \,\pi^* \eta \right ) &= \pi_* \left ( \underbrace{\defonde{f}{x}(x, t) dx \wedge \pi^* \eta}_{\text{type A}} + \underbrace{\defonde{f}{t}(x, t)\, dt \wedge \pi^* \eta}_{\text{type B}} + \underbrace{f(x, t) \, d\pi^* \eta}_{\substack{\text{type A because } \\ d\pi^*=\pi^*d}} \right ) \overset{(1)}{=} \\
&=\int_{-\infty}^{\infty} \defonde{f}{s}(x, s) ds \, \pi^* \eta = 0
\end{align*}
where (1) is true up to a sign (but in the end the sign doesn't matter), and we used the definition of $\pi_*$ for (type B) forms \eqref{typeb} and  the fact that $\pi_*(\text{type A}) = 0$. Finally, we used the fundamental theorem of integral calculus with the fact that $f$ has compact support.
...
\item $K$ is not a chain map (otherwise the right-hand side above would be 0).
\end{itemize}
\end{proposition}
\begin{proof}
...
\begin{remarkbox}
\begin{remark}
We proved the isomorphism in $H_c^k(M)$. Indeed, if $\omega$ is closed, then \eqref{invequation} becomes:
...
Now, we are finally ready to prove the compactly supported Poincaré lemma.

\begin{lemma}[Compactly supported Poincaré lemma] \label{compsupppoinclemma}
\begin{equation}
H_c^k(\rfield^n) \cong
\begin{cases*}
0, & $k \not = n$ \\
...
For $\fbox{n=1}$, the case $k \not = 1$ corresponds to the case $k=0$: we are considering closed and compactly supported 0-forms, i.e. functions which are 0 everywhere. Thus, the only closed and compactly supported 0-form which is not exact is the trivial one:  $H^0_c(\rfield) = 0$. As regards the case $k=1$, we prove that the following map is an isomorphism:
...
Now, we prove that I is surjective. Indeed, given a function $f > 0$ on $\rfield$ with compact support, let's consider $\eta = f(x) dx \in H^1_c(\rfield)$. We have that $\int_{\rfield} \eta = d > 0$. So, for every $c \in \rfield$ the form $\omega \equiv \frac{c}{d} \, \eta$ is such that $\int_{\rfield} \omega = c$. Thus, $I$ is surjective.

In order to prove that $I$ is injective, we just need to prove that $\ker I = \{0\}$ (since $I$ is linear). By the standard Poincaré lemma \eqref{poinclemma} we know that every closed 1-form on $\rfield$ is exact, i.e. $\omega = d\alpha$, but $\alpha$ might not be compactly supported!
...
\end{equation}
Let's suppose the claim is true for $n-1$, then we can consider $M=\rfield^{n-1}$ (notice that $\rfield^n$ is an oriented manifold) and thanks to the isomorphism \eqref{hisomorph} the claim is true also for $n$.
\end{proof}

\begin{remarkbox}
\begin{remark}
The Poincaré lemma \ref{compsupppoinclemma} is valid for $\rfield^n$, but it also holds for open subsets of manifolds which are diffeomorphic to $\rfield^n$. For instance, we can consider the convex subsets $U$ contained in a manifold. The form $\eta \in \Omega_c^{n-1}(U)$ with $d\eta=\omega$ produced in the proof of the Poincaré lemma for a compactly supported form $\omega$ can be extended by zero to a smooth form on the entire manifold. We will not make any distinction between the extended form and the form on $U$.
\end{remark}
\end{remarkbox}

We will now analyze some consequences of the compactly supported Poincaré lemma.

\begin{definition}[Bump form] \label{bumpform}
A bump form on $M^n$ (manifold of dimension $n$) is a compactly supported $n$-form $\omega = \rho \, dx^1 \wedge \ldots \wedge dx^n$ supported in a coordinate domain $(U, \varphi), \varphi \colon U \rightarrow \varphi(U) \subseteq \rfield^n $ so that $\left | \int_U \omega \right | = 1$.
\end{definition}

\begin{remarkbox}\begin{remark} [Cohomologous bump forms] \label{bumpremark}
We know that two forms are cohomologous in $H^k(U)$ if and only if they differ by an exact form (see remark \ref{cohomol}). The Poincaré lemma tells us that bump forms are cohomologous in $H^n_c(U)$ if their integral on $U$ is the same. Given $\omega, \omega'$ bump forms in $U$, then:
\begin{equation}
\omega = \omega' + d \eta,\, \eta \in \Omega_c^{n-1}(U) \Longleftrightarrow \int_U \omega = \int_U \omega'
\end{equation}
Indeed, the "$\Rightarrow$" is always true thanks to Stokes's theorem, whereas the "$\Leftarrow$" follows from the proof of the Poincaré lemma \ref{compsupppoinclemma}, because bump forms are $n$-forms in a $n$-dimensional manifold and the equality between the integrals implies that $[\omega] = [\omega']$ (because the isomorphism used in the proof is given by the integral map \eqref{mapn}). Thus, the two forms differ by an exact form, and such a form is compactly supported. 

In the next pages we will come across bump forms which are cohomologous to the $0$-form, even if their integrals are different. That is not a contradiction, since these compactly supported forms will be proved to be cohomologous in the cohomology $H^n(M)$ (and not $H^n_c(M)!$). i.e. their difference will be an exact $(n-1)$-form, but such a form will not be compactly supported. So, the above remark doesn't apply. 
\end{remark}\end{remarkbox}
...
\end{theorem}
\begin{proof}
Since $\partial M = \emptyset$, we have $\int_M (\omega + d \eta) = \int_M \omega$ by Stokes's theorem. So, the definition does not depend on the element of the equivalence class chosen. Now, let $c$ be a real number. Moreover, let $(U_i, \varphi_i)$  be a chart from the oriented atlas, and let's consider $\alpha=f(x_1, \ldots, x_n) dx^1 \wedge \ldots \wedge dx^n$ in $U_i$, with $f$ smooth function, $f > 0$. Then, $\int_M \alpha = d > 0$. Now, $\omega \equiv \frac{c}{d} \, \alpha \in H^{n}(M)$ and $\int_M \omega = c$. So, for every $c \in \rfield$ we can find a form $\omega$ such that $\int_M \omega = c$, i.e. the map is surjective.
\end{proof}
...
What if we remove the orientation property? Recall that we can't compute integrals without orientation.
\begin{theorem}
...
x &\mapsto \substack{\text{ intersection point with }S^{n-1} \\ \text{ of the ray passing through }x \text{ and } f(x)}
\end{align*}
(Notice that two intersection points can be considered: we define $F$ so that one of the two points is chosen for each $x$).
...
has to be injective because of $\oast$ (if the composition of maps is injective, the first map acting is injective). But this is impossible because $H^{n-1}(S^{n-1}) \cong \rfield$ (can be seen e.g. using the Mayer-Vietoris theorem) and $H^{n-1}(\overline{B_1(0)}) \cong 0$, with $B_1(0)$ $n$-dimensional ball. In order to prove the latter, we can't use the Poincaré lemma because $\overline{B_1(0)}$ is not homeomorphic to $\rfield^n$ (one is compact and the other is not, and compactness is left invariant under homeomorphisms). But we can prove that $\id \colon \overline{B_1(0)} \rightarrow \overline{B_1(0)}$ is homotopy equivalent to the map $\text{zero} \colon \overline{B_1(0)} \rightarrow \overline{B_1(0)}$ that sends every element of $\overline{B_1(0)}$ to $0 \in \overline{B_1(0)}$. Once we do this, we use the fact that the space itself is homotopy equivalent to the trivial set (for $n \ge 2$) and then the de Rham cohomology must be trivial as well.
\end{proof}
...
Let $f \colon M \rightarrow  N$ be smooth, let $N, M$ be closed, connected and orientable manifolds of the same dimension. Then, the degree of $f$ is
\begin{equation}
...
\begin{remarkbox}\begin{remark}
Definition \ref{degreemap} does not depend on the $\omega$ chosen: the proof is in the first lines of page 458 of \citetalias{Lee}. It is proved that the number $\deg f$ exists and the proof does not depend on $\omega$. Moreover, the degree of a map is homotopy invariant: if $f_0, f_1 \colon M \rightarrow N$ are two homotopic maps, then $f_0^* = f_1^*$, so $f_0$ and $f_1$ have the same degree.
\end{remark}\end{remarkbox}
...
\end{align*}
Then we have $\deg(f_k) = k$, indeed we can consider $\omega = d\varphi, \varphi$ angular coordinate on $S^1$ (cf. example \ref{angexample}). We stress that $d\varphi$ is not exact (it is just a bad notation) because $\varphi$ is not a well defined function (it is well defined up to a $2\pi$ angle). Then, we have:
...
\end{theorem}
\begin{proof}
Let us consider a regular value $p\in N$ of $f$. The value $p$ exists thanks to \href{https://en.wikipedia.org/wiki/Sard's_theorem}{Sard's theorem}. Sard's theorem states that the set of the critical values of a smooth function $f$ from a manifold to another one is a null-measure set. We can apply this theorem to our case (and whenever $f$ is only continuous we can approximate it by smooth functions). Indeed, if the set of critical values has zero measure, its complementary set (the regular values) has dimension $n$.
In particular, $f^{-1}(p)$ is a submanifold of dimension 0 by definition \ref{sub3}. So, $f^{-1}(p)$ consists of a finite collection of points $q_1, \ldots, q_m$. By the inverse function theorem, there exists a neighbourhood $U_i$ of $q_i \in f^{-1}(p)$ such that $\restrict{f}{U_i}$ is a diffeomorphism onto its image. Let $\omega$ be a bump form supported in $\bigcap_i f(U_i) = V$. Then:
$$\int_M f^* \omega = \sum\limits_i \underbrace{\int_{U_i} f^* \omega}_{\substack{=\pm \int_{f(U_i)} \omega \\ \text{depending} \\ \text{on orientation}}} = \left (\sum\limits_i  \pm 1 \right ) \int_V \omega $$
...
\section[Poincaré Duality]{\crule[blue!70!white]{0.3cm}{0.4cm}  Poincaré Duality}
...
$$f^* \omega = \underbrace{\left ( (\pr_1 \circ f)^* d \varphi_1 \right )}_{\text{closed 1-form on }S^2} \wedge \underbrace{\left( \pr_2 \circ f)^* d \varphi_2 \right)}_{\text{also closed}}$$
 due to proposition \ref{pullbackprop}, point 3, and to the naturality of the pullback. Now, $H^1(S^2)=0$ (not proved, but it can be computed using the Mayer-Vietoris theorem \ref{mvthm}). So, every closed form on $S^2$ is exact. Thus, the two closed 1-forms above are actually exact forms.
...
\end{theorem}
\begin{proof}
Let's cover $M$ with charts $(U_i, \varphi_i)$ and pick a subordinate partition of unity $\phi_j \colon V_j \subset U_{i(\ldots)}\rightarrow \rfield$. We can do that because our manifold is second-countable and paracompact (the latter one is actually a necessary hypothesis for the theorem). On each set $V_j$ there is a metric $g_j$ coming from the coordinates. Then, $g = \sum\limits_j \lambda_j g_j$ is a Riemannian metric (easy to verify). The key point is that the positive-definiteness property is preserved under convex combinations.
\end{proof}
...
A Riemannian metric assigns lengths to vectors, angles between vectors, etc. We want to extend the concept of metric (or, analogously, of Lorentzian metric) to alternating forms.
\newline
\begin{remarkbox}
...
\end{equation}
\end{lemma}
\begin{proof}[Proof of equation \eqref{deltaeqn}]
