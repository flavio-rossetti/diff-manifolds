...
\begin{definition}[Pullback]
  ...
  such that
  \begin{equation*}
    (f^* \omega)(p) (u_1, \ldots, u_k) = \omega (f(p)) (df_p(u_1), \ldots, df_p(u_k)), \quad \forall\, \, p \in \rfield^n, \forall \,\, u_i \in \mathfrak{X}(U) \, .
  \end{equation*}
\end{definition}
...
is a vector tangent to $\rfield^n$ at $p$. Since vectors are like directional derivatives, $v_p(f)$ is the directional derivative of $f$ with respect to $v_p$ (not evaluated at any point, for now!). In particular, if we apply the definition to function $h \in C^{\infty}(\rfield^n, \rfield^m)$, we have (cf. also p. 55 \citetalias{Lee}): 
$$(f_*)_p (v)(h) \overset{(1)}{=} v(h \circ f) = v(h(f)) \overset{(2)}{=} v (f^* h) $$
where we used: (1) $(f_*)_p (v)(h)  = v_p(h \circ f)$, it can be proved that it is the right expression in a similar way to what we did to prove that $(f_*)_p (v)= v_p(f)$ is the right expression (cf. remark \ref{diffcoordremark}), (2) the expression of the pullback for a differentiable function, which is completely legal since we defined it for differentiable $k$-forms, and a differentiable function is just a 0-form.
\end{remark}\end{remarkbox}
...
Then we can extend Poincaré lemma to $k$-forms:
...
\begin{remarkbox}\begin{remark}[Cohomologous forms]
  The set $\ker(d_{k+1})$ contains all the $k$ forms that are closed. The set $\im(d_k)$ contains the exact forms. As usual when we deal with vector spaces, the equivalence relation is:
...
\begin{tcolorbox}\begin{example}
  Let's consider the following examples:
  \begin{itemize}
    \item If $U$ is contractible, then $H^k(U, \rfield) = \{0\}$ by Poincaré lemma,
...
\begin{remarkbox}\begin{remark}
  ...
  If $\phi$ is a diffeomorphism, then the pushforward $\phi_*$ is an isomorphism and hence also $\phi^*$ due to $\phi^* = \phi_*^{-1}$.
...
\end{align*}
(Notice that: when both $x$ and $f(x)$ belong to the border of the sphere the intersection is not unique. In that case we choose the point $x$).
