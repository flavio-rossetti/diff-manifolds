...
&= f^* d \omega
\end{align*}
Where we used: (1) $f^*dx^i = df^i$ (i-th coordinate), the product rule and the fact that $d^2 = 0$, (2) given $h \equiv a_{i_1 \cdots i_k} \in C^1(\rfield^n)$, then $df^* h(z) = z(f^*h) = dh(f_* z) = f_*z(h) = f^*dh(z)$. \qedhere
\end{enumerate}
\end{proof}
...
\end{equation}
(Remember: $\phi\colon (t, p) \mapsto \phi(t, p),$ whereas $ \phi_t^*v \colon (t, p) \mapsto v(\phi (t, p))$).
\end{definition}
