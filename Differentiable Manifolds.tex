...
\item "$\subseteq$" follows from the fact that $[g] \in \faktor{G}{G^V}$ means that if $g \in G^V$, then we get the entire subgroup $G^V$. If $g \not \in G^V$, then we are considering the elements in $G \setminus G^V$. Since $g G^V$ means that we are multiplying $g$ for all possible element of $G^V$, and since $G^V$ contains the identity element (because it is a subgroup), we can get all the possible elements of $G \setminus G^V$. Then $G \subseteq \bigcup_{[g] \in \faktor{G}{G^V}} g G^V$
\end{itemize}
